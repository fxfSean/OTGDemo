...
public class PL2303HXDSimpleTest extends Activity {
	...
	private void writeDataToSerial() {
		...
		String test = new String ("060941416f414141414141414141424543450a");
		byte[] bytes = HexTools.hexStringToBytes(test);
		Log.i("test",HexTools.bytesToHexString(bytes,bytes.length));
		/*
...
//		int res = mSerial.write(strWrite.getBytes(), strWrite.length());
		...
		Toast.makeText(this, "Write length: "+test.length()+" bytes", Toast.LENGTH_SHORT).show();
	...
	private Runnable tLoop = new Runnable() {
		public void run() {	
			...
			Time t = new Time();
			byte[] rbuf = new byte[4096];    	
			final int mBRateValue[] = {115200};
			PL2303Driver.BaudRate mBRate[] = {PL2303Driver.BaudRate.B115200};
